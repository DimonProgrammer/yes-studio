...
const PREPS = new Set(['в','на','с','к','у','о','и','а','но','за','по','из','до','от','не','ни','же','бы','ли','об','при','без','для','под','над']);
function nb(text: string): string {
  // Non-breaking space after short prepositions
  let r = text.replace(/\S+\s/g, (m) => {
    const w = m.trimEnd();
    return PREPS.has(w.toLowerCase()) ? w + '\u00A0' : m;
  });
  // Non-breaking space before em-dash (prevents dash from starting a new line)
  r = r.replace(/ —/g, '\u00A0—');
  return r;
}

/** Parse <accent>text</accent> markers and return JSX with styled spans */
function hl(text: string): React.ReactNode {
  const parts = text.split(/(<accent>.*?<\/accent>)/g);
  if (parts.length === 1) return nb(text);
  return parts.map((part, i) => {
    const m = part.match(/^<accent>(.*?)<\/accent>$/);
    if (m) return <span key={i} className="text-accent">{nb(m[1])}</span>;
    return nb(part);
  });
}
...
function currentMonth() { return MONTHS_PREPOSITIONAL[new Date().getMonth()]; }
  ...
  return (
    <div className="border-b border-silk/10">
      <button onClick={onClick} className="w-full flex items-center justify-between py-6 md:py-7 text-left group cursor-pointer">
        <span className="font-display font-bold text-base md:text-lg text-silk group-hover:text-gold transition-colors pr-8">{nb(q)}</span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
            <p className="font-body text-base text-umber leading-relaxed pb-7 max-w-2xl">{nb(a)}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
export default function App() {
  ...
  const personalBrand = [
    { icon: Sparkles, title: 'Личный имидж', desc: 'Поможем сформировать <accent>уверенный и привлекательный имидж</accent> (укладки, мейки, одежда, аксессуары, стиль общения и поведения), который будет гармонировать с твоим характером.' },
    { icon: TrendingUp, title: 'Индивидуальная стратегия', desc: 'Разработаем <accent>уникальный план развития</accent> твоего личного бренда, который будет состоять из знаний, навыков, сильных качеств, стиля, особых фишек, идей и позиций.' },
    { icon: Camera, title: 'Создание контента', desc: '<accent>Бесплатно</accent> создадим профессиональный, привлекательный и запоминающийся фото и видео контент, который будет резонировать с твоими зрителями.' },
    { icon: BookOpen, title: 'Индивидуальное обучение', desc: 'Каждой модели предоставляется <accent>персональный куратор</accent>, который обучает всем тонкостям работы и будет сопровождать на всех этапах развития.' },
    { icon: Award, title: 'Карьерный рост', desc: 'Ты будешь участвовать в нашей программе карьерного развития, где есть <accent>повышения уровня</accent>, участия в специальных акциях и эксклюзивных шоу!' },
  ];

  const conditionItems = [
    { title: 'Работай когда удобно', desc: '<accent>Сама выбираешь график</accent>. Минимум <accent>4 часа</accent> в день. Можно совмещать с учёбой или основной работой.' },
    { title: 'Деньги каждый день', desc: '<accent>Ежедневные</accent> или еженедельные выплаты на карту любого банка. <accent>Без задержек</accent>.' },
    { title: 'Полная конфиденциальность', desc: 'Работа на английских площадках для зарубежной аудитории. <accent>Тебя не увидят знакомые</accent>.' },
    { title: 'Только NON-NUDE', desc: 'Наши модели <accent>не раздеваются</accent>. Позитивное онлайн-общение в дружеской атмосфере.' },
    { title: 'Оплачиваемая стажировка', desc: 'Попробуй несколько дней — мы <accent>оплатим стажировку</accent>, даже если решишь не продолжать.' },
    { title: 'Отпуск каждые 3 месяца', desc: 'Мы заботимся о балансе работы и отдыха. <accent>Регулярный оплачиваемый отдых</accent>.' },
  ];
  ...
  const testimonials = [
    { name: 'Алина', tenure: '6 месяцев', quote: 'Стало спокойнее — я знаю, что <accent>завтра будут деньги</accent>. Наконец-то не считаю дни до зарплаты.', photo: photos.testimonials[0] },
    { name: 'Дарья', tenure: '3 месяца', quote: 'Наконец-то понятно, что делать. <accent>Куратор объясняет каждый шаг</accent>, не чувствуешь себя одной.', photo: photos.testimonials[1] },
    { name: 'Кристина', tenure: '1 год', quote: 'За первый месяц заработала больше, чем за 3 на прошлой работе. Сейчас — <accent>свободный график и путешествия</accent>.', photo: photos.testimonials[2] },
  ];
  ...
  const successStories = [
    {
      ...
      after: '250 000 ₽/мес',
      quote: 'Первые 2 недели было страшно. Потом куратор помог разобраться, и уже <accent>на 3-й неделе я заработала больше</accent>, чем за месяц в офисе.',
    ...
    },
    {
      ...
      after: '180 000 ₽/мес',
      quote: 'Совмещаю с учёбой на 4 курсе. <accent>Свободный график</accent> — это то, чего мне не хватало. Теперь <accent>могу позволить себе</accent> то, о чём раньше мечтала.',
    ...
    },
    {
      ...
      after: '300 000 ₽/мес',
      quote: 'Пришла, когда студии было 5 лет. Сейчас я — топ-модель. <accent>Купила машину</accent>, путешествую каждый сезон. Главное — <accent>не бояться начать</accent>.',
      photo: photos.successStories[2],
    },
  ];

  const faqData = [
    { q: 'Сколько я буду зарабатывать?', a: 'Средняя зарплата — 150 000 ₽. На старте — от 90 000 ₽ при стандартном графике 5 смен по 7 часов. Топ-модели — от 300 000 ₽. Ограничений нет.' },
      ...
      <div className="overflow-x-hidden bg-noir text-silk grain">
          ...
          <div className="relative z-10 w-full lg:w-[55%] flex flex-col justify-between px-5 md:px-10 lg:px-16 pt-24 md:pt-28 pb-8 lg:pb-14">
            ...
            <div className="my-auto py-8 lg:py-10">
              <motion.p initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
                className="font-display font-bold text-[clamp(2rem,6.5vw,5.5rem)] leading-[0.88] tracking-tight text-silk mb-6"
              >
                ЕДИНСТВЕННАЯ
              ...
              >
                {nb('Зарабатывай ')}<span className="text-accent">на общении</span>{nb(' от ')}<span className="text-silk font-medium">90 000 ₽</span>{nb(' в месяц — ')}<span className="text-accent">без раздевания</span>{nb(', с ')}<span className="text-accent">личным куратором</span>{nb(' и ')}<span className="text-accent">ежедневными выплатами</span>{nb('. Средняя зарплата — ')}<span className="text-gold font-medium">150 000 ₽</span>.
              </motion.p>
          ...
          <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
            ...
            <Reveal delay={0.2}>
              <p className="font-body text-sm text-silk/40 text-center mt-6 max-w-lg mx-auto italic">
                {nb('Видео снято в нашей студии в Ростове, в Москве условия и подход к моделям такие же')}
              </p>
            </Reveal>
            ...
            <div className="text-center max-w-2xl mx-auto mb-12 md:mb-16">
              <Reveal>
                ...
                <p className="font-body text-base md:text-lg text-silk/60 leading-relaxed">
                  {nb('В большинстве студий модель просто сажают перед камерой — и ')}<span className="text-accent">дальше сама</span>{nb('. Нет образа, нет стратегии, нет поддержки. Результат — ')}<span className="text-accent">30–40 тысяч</span>{nb(' и ')}<span className="text-accent">выгорание</span>{nb(' через пару месяцев.')}
                  <br /><br />
                  <span className="text-accent">{nb('У нас иначе.')}</span>{nb(' Для каждой девушки мы создаём Personal Brand: ')}<span className="text-accent">индивидуальный образ</span>{nb(', стиль общения, ')}<span className="text-accent">стратегию роста</span>{nb(' и профессиональный контент. Поэтому средний доход наших моделей — ')}<span className="text-accent font-medium">150 000 ₽</span>{nb(', а не 40 000 ₽.')}
                </p>
              </Reveal>
            </div>
                  ...
                  <Reveal key={i} delay={i * 0.05} className={`${isFeatured ? 'lg:row-span-2' : ''} ${isWide ? 'lg:col-span-3' : ''}`}>
                    <div className={`glass-card rounded-2xl overflow-hidden h-full flex flex-col group hover:border-gold/20 transition-all duration-300 ${isWide ? 'lg:flex-row' : ''}`}>
                      ...
                      <div className={`p-6 ${isFeatured || isWide ? 'lg:p-8' : ''} ${isWide ? 'lg:flex-1' : ''}`}>
                        ...
                        <h3 className={`font-display font-bold text-silk mb-3 leading-tight ${isFeatured ? 'text-xl lg:text-2xl' : 'text-lg'}`}>{nb(item.title)}</h3>
                        <p className={`font-body text-silk/50 leading-relaxed ${isFeatured ? 'text-sm' : 'text-sm'}`}>{hl(item.desc)}</p>
                      </div>
                    </div>
                  </Reveal>
                );
              ...
              {guarantees.map((g, i) => (
                <Reveal key={i} delay={i * 0.05}>
                  <div className="text-center">
                    ...
                    <p className="font-display font-bold text-sm text-noir mb-1">{nb(g.title)}</p>
                    <p className="font-body text-xs text-noir/60 leading-relaxed">{nb(g.desc)}</p>
                  </div>
                </Reveal>
              ))}
            </div>
            <Reveal delay={0.4}>
              <div className="text-center mt-8">
                <button onClick={toForm} className="inline-flex items-center gap-2 bg-noir text-gold px-7 py-3 rounded-full font-display font-bold text-xs tracking-[0.1em] uppercase hover:bg-cocoa transition-colors cursor-pointer">
                  {nb('Попробуй с оплачиваемой стажировкой')} <ArrowRight size={14} />
                </button>
              </div>
            </Reveal>
          </div>
              ...
              {testimonials.map((t, i) => (
                <Reveal key={i} delay={i * 0.1}>
                  <div className="glass-card rounded-2xl p-7 h-full flex flex-col hover:border-gold/20 transition-all duration-300">
                    ...
                    <p className="font-body text-base text-silk/70 leading-relaxed flex-1 mb-6">«{hl(t.quote)}»</p>
            ...
            <Reveal delay={0.4}>
              <div className="text-center mt-10">
                <button onClick={toForm} className="group inline-flex items-center gap-2 text-gold font-display font-bold text-sm tracking-[0.08em] uppercase hover:gap-3 transition-all cursor-pointer">
                  {nb('Присоединяйся к нашей команде')} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            </Reveal>
          </div>
        ...
        <section className="bg-cocoa py-20 md:py-28">
          <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
            ...
            <div className="max-w-2xl mx-auto space-y-0">
              {[
                { step: '01', title: 'Заявка', desc: 'Оставляешь заявку на сайте или в Telegram. Перезвоним <accent>в течение часа</accent>.' },
                { step: '02', title: 'Знакомство', desc: 'Приезжаешь в студию на собеседование. <accent>Оплатим такси</accent>. <accent>Без обязательств</accent> — просто посмотришь, как всё устроено.' },
                { step: '03', title: 'Стажировка (3–5 дней)', desc: 'Пробуешь работать с куратором. Стажировка <accent>оплачивается</accent>. Если не понравится — уходишь <accent>с деньгами</accent>.' },
                { step: '04', title: 'Personal Brand', desc: 'Куратор создаёт твой образ: стиль общения, контент, стратегию развития. <accent>Бесплатная</accent> фотосессия и видео.' },
                { step: '05', title: 'Работа и доход', desc: 'Свободный график от 5 часов. <accent>Ежедневные выплаты</accent> на карту. Рост дохода по мере развития бренда.' },
              ].map((item, i) => (
                <Reveal key={i} delay={i * 0.08}>
                  <div className="flex gap-5 md:gap-8 group">
                    ...
                    <div className="pb-8 md:pb-10 min-w-0">
                      <h3 className="font-display font-bold text-lg text-silk mb-2 leading-tight">{nb(item.title)}</h3>
                      <p className="font-body text-sm text-silk/55 leading-relaxed">{hl(item.desc)}</p>
                    </div>
                  </div>
                </Reveal>
              ))}
          ...
          <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
              <Reveal className="lg:sticky lg:top-28">
                ...
                <p className="font-display font-medium text-lg text-silk/50 mb-8">{nb('Коротко о главном.')}</p>
                  ...
                  <Reveal key={i} delay={i * 0.08}>
                    <div className="glass-card rounded-2xl p-6 md:p-7 flex items-start gap-5 group hover:border-gold/20 transition-all duration-300">
                      ...
                      <div className="min-w-0">
                        <p className="font-display font-bold text-lg md:text-xl text-silk mb-1.5 leading-tight">{nb(item.title)}</p>
                        <p className="font-body text-base text-silk/55 leading-relaxed">{hl(item.desc)}</p>
                      </div>
                    </div>
                  </Reveal>
                ))}
        ...
        <section className="py-20 md:py-28">
          <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-4xl mx-auto">
              {[
                { icon: Sparkles, title: '«У меня нет модельной внешности»', text: 'Мы не проводим кастинги по внешности. Куратор помогает создать образ под тебя — укладка, стиль, подача. <accent>Внешность второстепенна</accent>, главное — <accent>общение</accent>.' },
                { icon: Shield, title: '«Это что-то стыдное»', text: 'Формат — <accent>NON-NUDE</accent>. Одежда, позитивное общение, дружеская атмосфера. Площадки — англоязычные, зарубежная аудитория. <accent>Тебя не увидят знакомые</accent>.' },
                { icon: Lock, title: '«А вдруг обманут»', text: 'Студия работает <accent>7 лет</accent>. Выплаты на карту <accent>каждый день</accent>. Стажировка оплачивается — даже если не останешься.' },
                { icon: Heart, title: '«У меня не получится»', text: 'С первого дня тебя ведёт персональный куратор. Обучение, поддержка, разбор ошибок. Большинство девушек выходят на <accent>стабильный доход</accent> уже через <accent>2–3 недели</accent>.' },
              ].map((card, i) => (
                <Reveal key={i} delay={i * 0.08}>
                  <div className="glass-card rounded-2xl p-7 h-full group hover:border-gold/20 transition-all duration-300">
                    ...
                    <h3 className="font-display font-bold text-lg text-silk mb-3 leading-tight">{nb(card.title)}</h3>
                    <p className="font-body text-sm text-silk/55 leading-relaxed">{hl(card.text)}</p>
                  </div>
                </Reveal>
              ))}
            </div>
            ...
            <Reveal delay={0.4}>
              <p className="font-body text-sm text-silk/40 text-center mt-8 max-w-md mx-auto">{nb('Так думали ')}<span className="text-accent">все наши девушки</span>{nb(', прежде чем прийти к нам.')}</p>
              <div className="text-center mt-5">
                <button onClick={toForm} className="group inline-flex items-center gap-3 bg-gold hover:bg-gold-dark text-noir px-8 py-4 rounded-full font-display font-bold text-sm tracking-[0.1em] uppercase transition-all cursor-pointer">
                  {nb('Попробуй — стажировка оплачивается')} <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            </Reveal>
          </div>
        ...
        <section className="py-20 md:py-28 overflow-hidden">
          <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
                  ...
                  <div className="space-y-5">
                    {transformationData.before.map((item, i) => (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-body text-xs text-silk/30 uppercase tracking-wider">{nb(item.label)}</p>
                          <p className="font-body text-base text-silk/50">{nb(item.value)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-5">
                    {transformationData.after.map((item, i) => (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-body text-xs text-gold/60 uppercase tracking-wider">{nb(item.label)}</p>
                          <p className="font-body text-base text-silk font-medium">{nb(item.value)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <Reveal delay={0.3}>
              <p className="font-body text-xs text-silk/30 text-center mt-8 italic">{nb('Цифры основаны на реальных результатах моделей Yes Studio.')}</p>
              <div className="text-center mt-5">
                ...
                <Reveal key={i} delay={i * 0.1}>
                  <div className="glass-card rounded-2xl overflow-hidden h-full flex flex-col hover:border-gold/20 transition-all duration-300 group">
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      ...
                      <p className="font-body text-sm text-silk/60 leading-relaxed flex-1">«{hl(story.quote)}»</p>
                    </div>
                  </div>
                </Reveal>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <Reveal className="md:col-span-2 md:row-span-2">
                <div className="glass-card rounded-2xl p-8 md:p-10 h-full flex flex-col justify-between min-h-[280px] md:min-h-[380px] relative overflow-hidden group hover:border-gold/15 transition-all">
                  ...
                  <div>
                    ...
                    <p className="font-body text-sm text-silk/50 leading-relaxed max-w-sm">{nb('Заработок от ')}<span className="text-accent">90 000 рублей</span>{nb(' в месяц, возможность выйти на ')}<span className="text-accent">150 000+ рублей</span>{nb('. ')}<span className="text-accent">Без ограничений</span>{nb(' по заработку.')}</p>
                  </div>
                  <div className="mt-8">
              ...
              <Reveal delay={0.05}>
                <div className="glass-card rounded-2xl p-7 h-full min-h-[170px] hover:border-gold/15 transition-all">
                  ...
                  <p className="font-body text-sm text-silk/50 leading-relaxed">{nb('Работа в формате онлайн-общения на международных площадках. Повышаешь ')}<span className="text-accent">навыки коммуникации</span>{nb(' и психологии.')}</p>
                </div>
              </Reveal>
              ...
              <Reveal delay={0.15}>
                <div className="glass-card rounded-2xl p-7 h-full min-h-[170px] hover:border-gold/15 transition-all">
                  ...
                  <p className="font-body text-sm text-silk/50 leading-relaxed">{nb('Знание английского ')}<span className="text-accent">не обязательно</span>{nb(' — установлен ')}<span className="text-accent">автоматический переводчик</span>{nb('. Через год будешь свободно разговаривать.')}</p>
                </div>
              </Reveal>
              ...
              <Reveal delay={0.2}>
                <div className="glass-card rounded-2xl p-7 h-full min-h-[170px] hover:border-gold/15 transition-all">
                  ...
                  <p className="font-body text-sm text-silk/50 leading-relaxed">{nb('Бесконечные комплименты, подарки и всевозможные знаки внимания и восхищения со стороны пользователей ')}<span className="text-accent">повышают самооценку</span>{nb(' всех наших моделей.')}</p>
                </div>
              </Reveal>
            </div>
          </div>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="font-body text-sm text-noir/70">{nb('Знаешь кому порекомендовать нашу компанию? Мы предлагаем вознаграждение ')}<span className="text-accent-dark font-medium">100$</span>{nb(', при условии, что модель заработает ')}<span className="text-accent-dark font-medium">300$</span>{nb(' — а ')}<span className="text-accent-dark">так может каждая</span>!</p>
                  </div>
                </div>
              ...
              <Reveal>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    'Нацелена на доход от <accent>150 000 рублей</accent> в месяц.',
                    'Имеешь <accent>желание учиться</accent> и развиваться.',
                    'Обладаешь коммуникабельностью, дисциплинированностью и энергичностью.',
                    'Стремишься выстроить <accent>долгосрочное сотрудничество</accent>.',
                  ].map((text, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <p className="font-body text-base text-silk/60 leading-relaxed">{hl(text)}</p>
                    </div>
                  ))}
                </div>
                <p className="mt-6 font-body text-sm text-gold font-medium">{nb('Не уверена? Запишись на ')}<span className="text-accent">оплачиваемую стажировку</span>{nb(' и попробуй!')}</p>
              </Reveal>
              <Reveal delay={0.15}>
        ...
        <section className="py-16 md:py-20">
          <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-4xl mx-auto">
              {[
                { icon: Banknote, title: 'Стажировка оплачивается', desc: 'Попробуй <accent>3–5 дней</accent>. Если не понравится — заберёшь деньги и уйдёшь. <accent>Без обязательств</accent>.' },
                { icon: MapPin, title: 'Такси за наш счёт', desc: 'Мы оплатим дорогу до студии на собеседование. <accent>Ничем не рискуешь</accent>.' },
                { icon: Star, title: 'Десятки девушек уже с нами', desc: 'Большинство из них <accent>тоже сомневались</accent>. Прочитай их истории выше.' },
              ].map((item, i) => (
                <Reveal key={i} delay={i * 0.1}>
                  <div className="glass-card rounded-2xl p-7 text-center h-full group hover:border-gold/20 transition-all duration-300">
                    ...
                    <h3 className="font-display font-bold text-base text-silk mb-2">{nb(item.title)}</h3>
                    <p className="font-body text-sm text-silk/55 leading-relaxed">{hl(item.desc)}</p>
                  </div>
                </Reveal>
              ))}
            ...
            <Reveal delay={0.4}>
              <div className="text-center mt-8">
                <button onClick={toForm} className="group inline-flex items-center gap-3 bg-gold hover:bg-gold-dark text-noir px-10 py-4 rounded-full font-display font-bold text-sm tracking-[0.1em] uppercase transition-all cursor-pointer">
                  {nb('Записаться — это бесплатно')}<ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            </Reveal>
          </div>
        ...
        <section id="apply" ref={applyRef} className="relative bg-cocoa py-20 md:py-28 overflow-hidden">
          ...
          <div className="relative max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
            <Reveal>
              <div className="flex items-center gap-2 mb-10 lg:mb-14 lg:justify-center">
                <div className="w-2 h-2 rounded-full bg-gold animate-pulse" />
                <p className="font-body text-sm text-gold font-medium">В {currentMonth()} осталось 3 из&nbsp;5 мест</p>
              </div>
            </Reveal>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center">
              <Reveal>
                ...
                <p className="font-body text-lg text-silk/50 leading-relaxed max-w-md mb-10">{nb('Оставь заявку — перезвоним ')}<span className="text-accent">за час</span>{nb(' и ')}<span className="text-accent">оплатим такси</span>{nb(' на собеседование.')}</p>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <span className="font-body text-sm text-silk/50">{nb(item.text)}</span>
                    </div>
                  ))}
                </div>
              </Reveal>
              ...
              <Reveal delay={0.15}>
                <form onSubmit={(e) => e.preventDefault()} className="glass-card rounded-2xl p-7 md:p-10 space-y-4">
                  ...
                  <button type="submit" className="w-full bg-gold hover:bg-gold-dark text-noir py-4 rounded-full font-display font-bold text-sm tracking-[0.1em] uppercase transition-colors flex items-center justify-center gap-3 mt-2 cursor-pointer">
                    {nb('Записаться — это бесплатно')}<ArrowRight size={18} />
                  </button>
                  <p className="font-body text-xs text-silk/25 text-center pt-1">{nb('Нажимая кнопку, вы даёте согласие на обработку персональных данных')}</p>
                </form>
              </Reveal>
            </div>
          </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={activeJob} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
                    ...
                    <div className="space-y-3">
                      {currentJob.requirements.map((text, i) => (
                        <div key={i} className="flex items-start gap-2.5">
                          ...
                          <p className="font-body text-sm text-silk/60 leading-relaxed">{nb(text)}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="mt-8 text-center">
                  <button onClick={toForm} className="group inline-flex items-center gap-3 bg-gold hover:bg-gold-dark text-noir px-10 py-4 rounded-full font-display font-bold text-sm tracking-[0.1em] uppercase transition-all cursor-pointer">
                    {nb('Откликнуться на вакансию')} <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                  </button>
                </div>
              </motion.div>
            </AnimatePresence>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
              <div className="lg:col-span-4">
                <Reveal>
                  ...
                  <p className="font-body text-sm text-silk/50 leading-relaxed">{nb('Не нашла ответ?')} <a href="https://t.me/studio_yes" className="text-gold hover:text-gold-dark transition-colors">{nb('Напиши в Telegram')}</a></p>
                </Reveal>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-6">
              <div className="md:col-span-5">
                ...
                <p className="font-body text-xs text-silk/30 leading-relaxed max-w-sm">
                  {nb('Студия Yes не является продавцом интимных услуг, а также не является посредником в их предоставлении. Работа моделей заключается в общении только в онлайн-пространстве и носит развлекательный характер. 18+')}
                </p>
              </div>
